...
var parseData = function(data) {
	switch(true) { // Temp 1
		case(data.state.reported.temp1 > 2000):
			data.state.reported['t1c'] = 'red';
			//sendAlert('Temp1', 'Red');
			break;
		case(data.state.reported.temp1 > 1500):
			data.state.reported['t1c'] = 'yellow';
		  break;
		default:
			data.state.reported['t1c'] = 'green';
	}
	switch(true) { // Temp 2
		case(data.state.reported.temp2 > 3000):
			data.state.reported['t2c'] = 'red';
			//sendAlert('Temp2', 'Red');
			break;
		case(data.state.reported.temp2 > 2500):
			data.state.reported['t2c'] = 'yellow';
			break;
		default:
			data.state.reported['t2c'] = 'green';
	}
	switch(true) { // Gate 1
		case(data.state.reported.gate1 % 2 == 1):
			data.state.reported['g1c'] = 'red';
			break;
		default:
			data.state.reported['g1c'] = 'green';
	}
	switch(true) { // Gate 2
		case(data.state.reported.gate2 % 2 == 1):
			data.state.reported['g2c'] = 'red';
			break;
		default:
			data.state.reported['g2c'] = 'green';
	}
	switch(true) { // Gate 3
		case(data.state.reported.gate3 % 2 == 1):
			data.state.reported['g3c'] = 'red';
			break;
		default:
			data.state.reported['g3c'] = 'green';
	}
	switch(true) { // Gate 4
		case(data.state.reported.gate4 % 2 == 1):
			data.state.reported['g4c'] = 'red';
			break;
		default:
			data.state.reported['g4c'] = 'green';
	}
}
